...
**Symptom:** A machine (washer, dryer) is shown as "running" in the UI, but the cycle is actually complete. A new order cannot be started on it.

**Fix:**
```bash
# Replace <MACHINE_ID> with the ID of the stuck machine
sqlite3 brain.db "UPDATE machine SET current_order_id = NULL, state = 'idle', cycle_started_at = NULL WHERE id = <MACHINE_ID>;"